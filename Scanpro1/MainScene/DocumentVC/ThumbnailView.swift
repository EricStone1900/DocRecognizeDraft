...
class ThumbnailView: UIView {
    ...
    var item: Any? {
        didSet {
            if let theItem = item as? DocFolderItem {
                let count = theItem.docFiles.count
                if count > 0 {
                    ...
                    let itemModel0 = theItem.docFiles[0]
                    let pageModel0 = itemModel0.docPages[0]
                    let imagepath = SHBFileTools.getNowFileFullPath(oldFullPath: pageModel0.fileFullPath!)
                    topLeftImageView.image = UIImage(contentsOfFile: imagepath)
            ...
            }else if let theItem = item as? DocFileItem{////DocFileItem
                let count = theItem.docPages.count
                if count > 0 {
                    // Hide/Show views
                    imageView.isHidden = false
                    ...
                    let firstPageitem = theItem.docPages[0]
                    let imagepath = SHBFileTools.getNowFileFullPath(oldFullPath: firstPageitem.fileFullPath!)//firstPageitem.fileFullPath!
