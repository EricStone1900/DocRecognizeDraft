...
static bool
GetIntersection(const cv::Vec4i &line_a, const cv::Vec4i &line_b, cv::Point &intersection) {
...
static bool
GetSegmentIntersection(const cv::Vec4i &line_a, const cv::Vec4i &line_b, cv::Point &intersection) {
    std::array<int, 3> pa{{line_a[0], line_a[1], 1}};
    std::array<int, 3> pb{{line_a[2], line_a[3], 1}};
    std::array<int, 3> la = Cross(pa, pb);
...
static bool IsSegmentsHasSameSegment(const std::vector<cv::Vec4i> segments, const cv::Vec4i segment,
                                     int image_width) {
...
static bool IsRectCornersReasonable(std::vector<Corner> rect_corners, int image_width) {
    ...
    cv::Vec4i segment_0_to_1 = cv::Vec4i(rect_points[0].x, rect_points[0].y, rect_points[1].x,
                                         rect_points[1].y);
    cv::Vec4i segment_1_to_2 = cv::Vec4i(rect_points[1].x, rect_points[1].y, rect_points[2].x,
                                         rect_points[2].y);
    cv::Vec4i segment_2_to_3 = cv::Vec4i(rect_points[2].x, rect_points[2].y, rect_points[3].x,
                                         rect_points[3].y);
    cv::Vec4i segment_3_to_0 = cv::Vec4i(rect_points[3].x, rect_points[3].y, rect_points[0].x,
                                         rect_points[0].y);
    ...
    double ratio1 = MIN(distance_of_0_to_1, distance_of_2_to_3) /
    MAX(distance_of_0_to_1, distance_of_2_to_3);
    double ratio2 = MIN(distance_of_1_to_2, distance_of_3_to_0) /
    MAX(distance_of_1_to_2, distance_of_3_to_0);
...
std::tuple<bool, std::vector<cv::Point>, std::vector<cv::Mat> >
ProcessEdgeImage(cv::Mat edge_image, cv::Mat color_image, bool draw_debug_image) {
    ...
    //<2>找线段
    cv::Mat binary_image;
    threshold(gray_image, binary_image, 0, 255,
              cv::THRESH_BINARY | cv::THRESH_OTSU); //HoughLinesP的输入 mat 是二值化的,前景背景分割
    ...
    for (int i = 0; i < linesP.size(); i++) {
        ...
        if (segment_length > kHoughLinesPMinLinLength) {
            ref_line_and_segment_pairs.push_back(std::make_tuple(ref_line, segment));  //参考线和实际线的直线对
        }
    }
    
    //<4>合并临近的直线
    std::vector<std::tuple<RefLineVec4i, cv::Vec4i> > merged_ref_line_and_segment_pairs = MergeRefLineAndSegmentPairs(
                                                                                                                      ref_line_and_segment_pairs, edge_image.cols, edge_image.rows);
    ...
    for (int i = 0; i < ref_line_and_segment_pairs.size(); i++) {
        ...
        ref_lines.push_back(ref_line);  //复制到ref_line和segments里??? 为啥?
    ...
    //<5>寻找segment线段的交叉点以及过滤  这里的复杂度是n^2,必须限制点直线的数量,如果分割结果不好,这里有点耗时
    std::vector<cv::Point> all_corners;
    std::vector<Corner> corners;
    int num_lines = std::min(int(segments.size()), maxLines);
    ...
    if (average_corners.size() >= 4) {
        ...
        int points_num = std::min(int(average_corners.size()), maxPoints);
        //4重循环的计算量还是有点大
        for (int i = 0; i <= points_num - 4; i++) {
            for (int j = i + 1; j <= points_num - 3; j++) {
                for (int m = j + 1; m <= points_num - 2; m++) {
                    for (int n = m + 1; n <= points_num - 1; n++) {
...
//    cv::Mat colorMat_ori(cv::Size(Wsrc,Hsrc),CV_8UC3);
//    //赋值
//    int shape_src[3];
//    shape_src[0]=Hsrc,shape_src[1]=Wsrc,shape_src[2]=3;
//    __copyArrayToMat(pSrcImg,shape_src,colorMat_ori);
//
//    //换算回原图大小
//    cv::Point2f incor[3];
//    for(int i=0;i<3;++i){
//        incor[i].x = ppoints[2*i]*(float)Wsrc/(float)Wedge;
//        incor[i].y = ppoints[2*i+1]*(float)Hsrc/(float)Hedge;
//    }
//
//    cv::Point2f outcor[3];
//    outcor[0] = cv::Point2f(0,0);
//    outcor[1] = cv::Point2f(Wdst,0);
//    outcor[2] = cv::Point2f(Wdst,Hdst);
//
//    auto M = cv::getAffineTransform(incor,outcor);
//    cv::Mat aff_img;
//    cv::warpAffine(colorMat_ori,aff_img,M,cv::Size(Wdst,Hdst));
//
//    //再转成指针输出
//    int shape_out[3];
//    shape_out[0]=Hdst,shape_out[1]=Wdst,shape_out[2]=3;
//    __copyMatToArr(aff_img,pDstImg,shape_out);
//}
void __copyArrayToMat(unsigned char *pimage, int *shape, cv::Mat &imageMat) {
    //遍历
    int cols = imageMat.size().width;
    int rows = imageMat.size().height;
    int channal = imageMat.channels();
    // std::cout<<rows<<","<<cols<<","<<channal<<std::endl;
    // std::cout<<shape[0]<<","<<shape[1]<<","<<shape[2]<<std::endl;
    
    assert(shape[0] == rows);
    assert(shape[1] == cols);
    assert(shape[2] == channal);
    
    for (int i = 0; i < rows; ++i) {
        unsigned char *pdatarow = (pimage + i * cols * channal);
        unsigned char *prow = imageMat.ptr<u_char>(i);
        for (int j = 0; j < cols * channal; ++j) {
            prow[j] = pdatarow[j];
        }
    }
}
...
void __copyMatToArr(cv::Mat &imageMat, unsigned char *pimage, int *shape) {
    //遍历
    const int cols = imageMat.size().width;
    const int rows = imageMat.size().height;
    const int channal = imageMat.channels();
    // std::cout<<rows<<","<<cols<<","<<channal<<std::endl;
    // std::cout<<shape[0]<<","<<shape[1]<<","<<shape[2]<<std::endl;
...
cv::Mat getAffImageTest2(cv::Mat src, std::vector<cv::Point2f> corners,int outW, int outH) {
    ...
    cv::Mat quad = cv::Mat::zeros(outH, outW, CV_8UC3);
